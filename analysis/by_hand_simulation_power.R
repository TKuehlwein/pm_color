...
setwd("/Users/tobiaskuehlwein/pm_color/analysis")

# load in pilot data file
results <- read.csv("data_all", header = TRUE)
...
sim_test   <-  simulation |>  
  group_by(trial, load) %>%
  summarise(color_avg = mean(color_abs_deviation, na.rm = TRUE),
            color_median = median(color_abs_deviation, na.rm = TRUE),
            color_dif_sd = sd(color_abs_deviation, na.rm = TRUE),
...
# run t-tests to check data for color.
...
# define model
fit <- lmer(color_abs_deviation ~ load * trial + (1|participant)
              , data = simulation)
summary(fit)
anova(fit)
summ(fit)
...
artificial_lmer <- makeLmer(formula = color_abs_deviation ~ load * trial + (1|participant), fixef = fixed_effects, 
                            VarCorr = random_variance, sigma = sigma, data = simulation)
summary(artificial_lmer)

# simulation parameters
steps <- c(350,370,390,410,420,450)
critical_value <- 2.5
n_sim <- 500
SESOI <- fixed_effects
...
simulation$participant <- as.numeric(simulation$participant)

# run simulation
power1 <- mixedpower(model = artificial_lmer, data = simulation,
                     fixed_effects = fixed_effects, simvar = "participant", steps = steps,
                     critical_value = critical_value, n_sim = n_sim, SESOI = SESOI)
power1
multiplotPower(power1)

# save power analysis
write.csv(power1, file = "power_stm_0.03_90%_pure.csv", row.names = TRUE)
